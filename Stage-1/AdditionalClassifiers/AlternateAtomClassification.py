# Atom classification - 1.1
# Implementation using functional programming
# Inclusion of Sample Atoms Classifier
# inclusion of Intergrity checker
from BlockClassification import ClassifyBlock
from IntegrityChecker import integrity_checker
from SupportFunctions import litEnd, bigEnd, hex_to_ascii, dprint
from HexData import data
from AtomDictionaries import ftyp, free, mdat, moov, mvhd, trak, tkhd, mdia, mdhd, hdlr, minf, smhd, vmhd, dinf, dref, url, stbl, stsd, stts, stss, stsc, stsz, stco, ctts, sdtp, stps, cslg, udta
from Classifiers import ftyp_classifier, udta_classifier, free_classifier, mdat_classifier, moov_classifier, mvhd_classifier, trak_classifier, tkhd_classifier, mdia_classifier, mdhd_classifier, hdlr_classifier, minf_classifier, vmhd_classifier, smhd_classifier, dinf_classifier, dref_classifier, sample_atom_classifier

d={'ftyp':ftyp, 'free':free, 'mdat':mdat, 'moov':moov, 'mvhd':mvhd, 'trak':trak, 'tkhd':tkhd, 'mdia':mdia, 'mdhd':mdhd, 'hdlr':hdlr, 'minf':minf, 'smhd':smhd, 'vmhd':vmhd, 'dinf':dinf, 'dref':dref, 'url':url, 'stbl':stbl, 'stsd':stsd, 'stts':stts, 'stss':stss, 'stsc':stsc, 'stsz':stsz, 'stco':stco, 'ctts':ctts, 'sdtp':sdtp, 'stps':stps, 'cslg':cslg, 'udta':udta}

try:
    file_size = ClassifyBlock()
    hex_pointer = 0
    origin_pointer = 0
    while (hex_pointer < (file_size*2)):
        cur_size, cur_atom = integrity_checker(hex_pointer,data)
        if cur_atom == "ftyp" or cur_atom == "free" or cur_atom == "mdat":
            while (cur_atom == "ftyp" or cur_atom == "free" or cur_atom == "mdat"):
                if (cur_atom == "ftyp"):
                    hex_pointer = ftyp_classifier(hex_pointer, cur_size,data)
                    print("\n", "File type(ftyp) Atom Details are:")
                    dprint(ftyp)
                elif (cur_atom == "free"):
                    hex_pointer = free_classifier(hex_pointer, cur_size,data)
                    print("\n", "Free type(free) Atom Details are:")
                    dprint(free)
                elif (cur_atom == "mdat"):
                    hex_pointer = mdat_classifier(hex_pointer, cur_size,data)
                    print("\n", "Media data(mdat) Atom Details are:")
                    dprint(mdat)
                if hex_pointer < (file_size*2):
                    cur_size, cur_atom = integrity_checker(hex_pointer,data)
                else:
                    break
            if hex_pointer >= (file_size*2):
                break

        if (cur_atom == "moov"):
            origin_pointer = moov_classifier(hex_pointer, cur_size,data)
            hex_pointer += 16
            print("\n", "Movie type(moov) Atom Details are:")
            dprint(moov)
        elif (cur_atom == "mvhd"):
            hex_pointer = mvhd_classifier(hex_pointer, cur_size,data)
            print("\n", "Moovie header(mvhd) Atom Details are:")
            dprint(mvhd)
        elif (cur_atom == "trak"):
            origin_pointer = trak_classifier(hex_pointer, cur_size,data)
            hex_pointer += 16
            print("\n", "Track type (trak) Atom Details are:")
            dprint(trak)
        elif (cur_atom == "tkhd"):
            hex_pointer = tkhd_classifier(hex_pointer, cur_size,data)
            print("\n", "Track header (tkhd) Atom Details are:")
            dprint(tkhd)
        elif (cur_atom == "mdia"):
            origin_pointer = mdia_classifier(hex_pointer, cur_size,data)
            hex_pointer += 16
            print("\n", "Movie Media (mdia) Atom Details are:")
            dprint(mdia)
        elif (cur_atom == "mdhd"):
            hex_pointer = mdhd_classifier(hex_pointer, cur_size,data)
            print("\n", "Movie Media Header(mdhd) Atom Details are:")
            dprint(mdhd)
        elif (cur_atom == "hdlr"):
            hex_pointer = hdlr_classifier(hex_pointer, cur_size,data)
            print("\n", "Handler Description (hdlr) Atom Details are:")
            dprint(hdlr)
        elif (cur_atom == "minf"):
            origin_pointer = minf_classifier(hex_pointer, cur_size,data)
            hex_pointer += 16
            print("\n", "Media Information (minf) Atom Details are:")
            dprint(minf)
        elif (cur_atom == "smhd"):
            hex_pointer = smhd_classifier(hex_pointer, cur_size,data)
            print("\n", "Sound Media Header (smhd) Atom Details are:")
            dprint(smhd)
        elif (cur_atom == "vmhd"):
            hex_pointer = vmhd_classifier(hex_pointer, cur_size,data)
            print("\n", "Video Media Header (vmhd) Atom Details are:")
            dprint(vmhd)
        elif (cur_atom == "dinf"):
            origin_pointer = dinf_classifier(hex_pointer, cur_size,data)
            hex_pointer += 16
            print("\n", "Data information (dinf) Atom Details are:")
            dprint(dinf)
        elif (cur_atom == "dref"):
            hex_pointer = dref_classifier(hex_pointer, cur_size,data)
            print("\n", "Video Media Header (dref) Atom Details are:")
            dprint(dref)
            # hex_pointer+=16
            # Dets of dref and url atoms are printed in the classifier due to an issue:
            # The last attribute of dref is data references which is a compilation of URL atoms
        elif (cur_atom == "stbl"):
            tempo = ""
            for j in range(hex_pointer, hex_pointer+8):
                tempo += data[j]
            stbl['Size'] = bigEnd(tempo)
            print("\nSample Table (stbl) details:")
            dprint(stbl)
            hex_pointer += 16
        elif (cur_atom == 'stsd' or cur_atom == 'stsc' or cur_atom == 'stts' or cur_atom == 'stss'
              or cur_atom == 'stsz' or cur_atom == 'stco' or cur_atom == 'ctts' or cur_atom == 'sdtp'
              or cur_atom == 'stps' or cur_atom == 'cslg'):
            hex_pointer = sample_atom_classifier(
                cur_atom, hex_pointer, cur_size,data)
            print("\n", cur_atom+" Atom Details are:")
            dprint(d[cur_atom])
        elif (cur_atom == "udta"):
            hex_pointer = udta_classifier(cur_atom,hex_pointer, cur_size,data)
            print("\n","User Data (udta) Atom Details are:")
            dprint(udta)
        else:
            hex_pointer += cur_size*2
    print("Final hex pointer value", hex_pointer)
except:
    pass
